...
async fn get_basemap_config(
    ...
    if let Some(summary) = context.maps().managed_basemap_summary().await? {
        return Ok(Json(config_from_managed_summary(summary)));
    }
...
fn config_from_managed_summary(summary: ManagedBasemapSummary) -> BasemapConfigResponse {
    BasemapConfigResponse {
        enabled: true,
        style_url: Some("/api/basemap/style.json".to_owned()),
